...
def test_custom_op(feedMat, corpus, chars, wordChars):
    ...
    decode = word_beam_search_module.word_beam_search(
        mat,
        25,
        "Words",
        0.0,
...
def test_mini_example():
    ...
    chars = (
        "ab "  # the first three characters which occur in the matrix (in this ordering)
    )
    word_chars = (
        "ab"  # whitespace not included which serves as word-separating character
    )
    mat = np.array(
