...
class Preprocessor:
    ...
    def _simulate_text_line(self, batch: Batch) -> Batch:
        ...
        for i in range(batch.batch_size):
            ...
            num_words = (
                random.randint(1, 8) if self.data_augmentation else default_num_words
            )
            ...
            for curr_sel_img, curr_word_sep in zip(sel_imgs, word_seps):
                ...
                target[
                    y : y + curr_sel_img.shape[0] :, x : x + curr_sel_img.shape[1]
                ] = curr_sel_img
    ...
    def process_img(self, img: np.ndarray) -> np.ndarray:
        ...
        if self.data_augmentation:
            ...
            if random.random() < 0.25:
                img = np.clip(
                    img + (np.random.random(img.shape) - 0.5) * random.randint(1, 25),
                    0,
                    255,
                )
        ...
        # no data augmentation
            ...
            img = cv2.warpAffine(
                img, M, dsize=(wt, ht), dst=target, borderMode=cv2.BORDER_TRANSPARENT
            )
    ...
    def process_batch(self, batch: Batch) -> Batch:
        ...
        res_gt_texts = [
            self._truncate_label(gt_text, max_text_len) for gt_text in batch.gt_texts
        ]
        return Batch(res_imgs, res_gt_texts, batch.batch_size)
